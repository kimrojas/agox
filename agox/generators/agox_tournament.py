...
class AGOXGenerator(GeneratorBaseClass):
    ...
    def _copy_candidates_from_main_database_to_database(self, samplable_candidates):
        if self.recalc:
            for i,outer_candidate in enumerate(samplable_candidates):
            ...
            [self.database.store_candidate(candidate, dispatch=False) for candidate
             in samplable_candidates]

    def _copy_strucs_for_inner_database(self, strucs_for_inner_database):
        for struc in strucs_for_inner_database:
    ...
    def get_candidates(self, sample, environment, nonsamplable_candidates, samplable_candidates):
        self.database.reset()

        # Add the data from the main database, such that it can be used for sampling/training/etc.
        self._copy_candidates_from_main_database_to_database(samplable_candidates)
        self._copy_strucs_for_inner_database(nonsamplable_candidates)
    ...
    def get_gofee_generator(cls, environment, database, calculator, iterations=25, 
        ...
        return cls(modules=internal_agox_modules, database=internal_database, 
                   main_database=database, iterations=iterations, template=environment.get_template())
    ...
    def __call__(self, sampler, environment, nonsamplable_candidates, samplable_candidates):
        return self.get_candidates(sampler, environment, nonsamplable_candidates, samplable_candidates)
