...
Run-details: Interactive to see the structure, energy etc, slider for episode and restart, dropdown for directory. 
Greedy builds: Interactive plot that shows 10 greedy-policy builds with slider for restart and episode and dropdown for directory.
...
headline = """
# Automagic Asla Analysis
This is an automatically generated ASLA analysis notebook.\n 
...
from agox.utils.jupyter_interactive import InteractiveStructureHistogram, InteractiveSuccessStats, InteractiveEnergy
"""
...
load_block = """
asla_analysis = Analysis()
force_reload = False
"""
for directory in directories:
    load_block += 'asla_analysis.add_directory({}, force_reload=force_reload)\n'.format(directory)
load_block += 'asla_analysis.compile_information()\n'
load_block += 'asla_analysis.calculate_CDF()\n'
...
success_block = """
ISS = InteractiveSuccessStats(asla_analysis)
...
energy_int_block = """
IE = InteractiveEnergy(asla_analysis)
...
interactive_struct_hist = """
ISH = InteractiveStructureHistogram(asla_analysis)
num_structures = np.sum(asla_analysis.restarts)
...
success_block = """
succes_fig, succes_ax = plt.subplots({})
asla_analysis.plot_CDF(succes_ax)
plt.show()
""".format(figsize)
add_cell(success_block, code)
...
histogram_headline = """
## Succes histogram"""
add_cell(histogram_headline, markdown)

histogram_block = """
histogram_fig, histogram_ax = plt.subplots({})
asla_analysis.plot_histogram(histogram_ax)
plt.show()""".format(figsize)
add_cell(histogram_block, code)

# Energy convergence:
energy_headline = """
## Energies
"""
add_cell(energy_headline, markdown)

energy_block = """
energy_fig, energy_ax = plt.subplots({})
asla_analysis.plot_energy(energy_ax)
plt.show()""".format(figsize)
add_cell(energy_block, code)
