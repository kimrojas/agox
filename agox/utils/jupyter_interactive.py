...
class InteractiveSuccessStats:
    ...
    def __init__(self, analysis):
        ...
        self.xmin = widgets.BoundedIntText(value=0, min=0, max=self.analysis.max_num_episodes, step=1, description='Min episodes:')
        self.xmax = widgets.BoundedIntText(value=self.analysis.max_num_episodes, min=0, max=self.analysis.max_num_episodes, step=1, description='Max episodes:')
        ...
        self.rate_button = widgets.ToggleButton(value=False, description='Ep. for rate', button_style='', tooltip='Control whether or not episode for rate is plotted', icon='check')
        ...
        self.widget_dict = {'labels':self.selector, 'xmin':self.xmin, 'xmax':self.xmax, 'uncertainty':self.uncertainty_button, 
                            'plot_labels':self.label_button, 'plot_episode_for_rate':self.rate_button, 'rate':self.rate_input, 
    ...
    def plot_succes_curve(self, ax, index, uncertainty=True):
        ...
        i = index
        max_episodes = np.max(self.analysis.episodes[i])
        if not (self.analysis.episodes[i] == max_episodes).all():
            idx = np.argmin(np.abs(
                np.array(self.CDF[i][0]).reshape(-1, 1) - np.array(self.analysis.episodes[i]).reshape(1, -1)
                ), axis=0)
            x = self.CDF[i][0][idx]
            x[x == self.CDF[i][0][-2]] = np.array(self.analysis.episodes[i])[x == self.CDF[i][0][-2]]
            ax.plot(x, self.CDF[i][1][idx], 'x', color=color)


        #max_episodes = np.max(self.analysis.episodes[index])
        #if not (self.analysis.episodes[index] == max_episodes).all():
        #    idx = np.argmin(np.abs(np.array(self.CDF[index][0]).reshape(-1, 1)-np.array(self.analysis.episodes[index]).reshape(1, -1)), axis=0)
        #    x = self.CDF[index][0][idx]
        #    x[x == self.CDF[index][0][-2]] = np.array(self.analysis.episodes[index])[x == self.CDF[index][0][-2]]
        #    ax.plot(x, self.CDF[index][1][idx], 'x', color=color)

    def plot_rate(self, ax, episode, rate, index):
        color = self.colors[index % len(self.colors)]
        ax.plot([episode, episode], [0, rate], '--', color=color)

    def update_plot(self, labels, xmin, xmax, uncertainty=True, plot_labels=True, plot_episode_for_rate=False, rate=0.5, save=False, save_name=None):
        fig, ax = plt.subplots(figsize=self.figsize)
        
        if plot_episode_for_rate:
            episode_for_rate = self.analysis.get_episodes_for_rate(rate)
        ...
        for i, label in enumerate(self.analysis.labels):
            if label in labels:
                ...
                if plot_episode_for_rate:
                    self.plot_rate(ax, episode_for_rate[i], rate, i)

        ax.set_xlabel('Episodes', fontsize=self.label_fontsize, color=FONTCOLOR)
...
class InteractiveEnergy:
    ...
    def update_plot(self, labels, xmin, xmax, ymin, ymax, plot_mean, plot_limit, rolling_average, policy_only, save, save_name):
        ...
        episodes = np.arange(energies.shape[-1])

        if policy_only:
            episodes = np.arange(5, energies.shape[-1], 5)
            energies = energies[:, :, episodes-1]
    ...
    def plot_mean_energy(self, ax, index, policy=False):
        ...
        if policy:
            ax.plot(episodes, arr, linestyle='-', label='_nolegend_',
    ...
    def plot_energy_limits(self, ax, index, w, policy=False):
        ...
        ax.fill_between(episodes, lower_limit, upper_limit, color=self.colors[index], alpha=0.4)