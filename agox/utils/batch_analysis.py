...
from agox.utils.db_tools import get_best
...
class Analysis:
    ...
    def __init__(self, directories=[], dE=0.05, delta_energy=None, labels=None, save_data=True, force_reload=False, 
        ...
        if len(histogram_kwargs) == 0:
            self.histogram_kwargs = {'min_hist_episode':0, 'max_hist_episode':0, 'operation':'best', 'threshold':1000}
        else:
    ...
    def compile_information(self):
        ...
        for i in range(self.num_directories):
            # self.episodes[i] = np.max([len(arr) for arr in energies_list[i]])
            self.episodes.append([len(arr) for arr in self.energies_list[i]])
            self.restarts[i] = len(self.energies_list[i])
            
        self.max_num_restarts = np.max(self.restarts).astype(int)
        self.max_num_episodes = np.max([np.max(episodes) for episodes in self.episodes]).astype(int)

        self.energies = np.zeros((self.num_directories, self.max_num_restarts, self.max_num_episodes)) + self.zero_offset
        self.best_energies = np.zeros((self.num_directories, self.max_num_restarts, self.max_num_episodes)) + self.zero_offset
        ...
        # Figure out which episode in each run produced the best episode:
    ...
    def analyse_directories_old(self):        
        ...
        # self.episodes = np.zeros(self.num_directories).astype(int)
        self.episodes = []
        self.restarts = np.zeros(self.num_directories).astype(int)
        for i in range(self.num_directories):
            # self.episodes[i] = np.max([len(arr) for arr in energies_list[i]])
            self.episodes.append([len(arr) for arr in energies_list[i]])
            self.restarts[i] = len(energies_list[i])

        self.max_num_restarts = np.max(self.restarts).astype(int)
        self.max_num_episodes = np.max(np.max([episodes for episodes in self.episodes])).astype(int)

        self.energies = np.zeros((self.num_directories, self.max_num_restarts, self.max_num_episodes))
        self.best_energies = np.zeros((self.num_directories, self.max_num_restarts, self.max_num_episodes))
        ...
        self.num_atoms = best_structure.get_number_of_atoms()
    ...
    def calculate_CDF(self):
        ...
        #times_np[(times_np == 0) * (events_np == 0)] = self.max_num_episodes - 1
        times_np[(events_np == 0)] = self.max_num_episodes - 1
    ...
    def plot_CDF(self, ax, factor=1):
        ...
        for i in range(self.num_directories):
            ...
            max_episodes = np.max(self.episodes[i])
            if not (self.episodes[i] == max_episodes).all():
                idx = np.argmin(np.abs(
                    np.array(self.CDF[i][0]).reshape(-1, 1) - np.array(self.episodes[i]).reshape(1, -1)
                    ), axis=0)
                x = self.CDF[i][0][idx]
                x[x == self.CDF[i][0][-2]] = np.array(self.episodes[i])[x == self.CDF[i][0][-2]]
        ...
        ax.set_ylim([0, 1])
        ax.set_xlim([0, self.max_num_episodes])
        ax.legend()
    ...
    def plot_histogram(self, ax, histogram_kwargs=None):
        ...
        if histogram_kwargs is None:
            min_hist_episode = self.histogram_kwargs['min_hist_episode']
            max_hist_episode = self.histogram_kwargs['max_hist_episode']
            operation = self.histogram_kwargs['operation']
            threshold = self.histogram_kwargs['threshold']
        else:
            min_hist_episode = histogram_kwargs['min_hist_episode']
            max_hist_episode = histogram_kwargs['max_hist_episode']
        ...
        if max_hist_episode == 0:
            max_hist_episode = self.energies.shape[2]-1

        if threshold != 0:
            threshold = self.global_best_energy + threshold 

        # Apply 'operation':
        if operation == 'best': # Histogram over the energies of the BEST structures pr. restart (so min over episodes dimension effectively)
            data = self.best_energies[:, :, max_hist_episode-1]
        elif operation == 'all': # Histogram over the energies of ALL structures pr. episodes within the episode limits.
            data = self.energies[:, :, min_hist_episode:max_hist_episode]
    ...
    def print_info(self, print_all):
        for num, directory in enumerate(self.directories):
            finished = (self.episodes[num] == np.max(self.episodes[num])).all()
    ...
    def plot_energy_distribution(self, ax, energy_range = 50, episode_start=100, episode_end=101, best=True): # New
        """
        ...
        episode_start - int: Starting episode to consider.
        episode_end - int: Ending episode to consider. 
        ...
        Parameters: The parameters are pretty arbitary at the moment, it would be nice if the width of the gaussian was related to dE in some 
                    ...
                    With the correct ratio between the amplitude and the width the integral could num_restart * num_episodes, at which point a 
        ...
        for d in range(self.num_directories):
            if not best:
                D[d] += gaussian(E, self.energies[d, 0:self.restarts[d], episode_start:episode_end])
            else:
    ...
    def get_episodes_for_rate(self, rate): # New
        CDF = np.array(self.CDF)
        episode_for_rate = np.zeros(len(CDF))
        for j in range(len(CDF)):
            index = np.argmin(np.abs(CDF[j, 1]-rate))
            episode_for_rate[j] = CDF[j, 0][index]

        return episode_for_rate
...
if __name__ == '__main__':
    ...
    # Histogram settings:
    parser.add_argument('-hgl', '--histogram_limits', nargs='+', default=[0, 0]) # Histogram limits: min episode, max episode
    ...
    histogram_kwargs = {
                        'min_hist_episode':int(args.histogram_limits[0]), 
                        'max_hist_episode':int(args.histogram_limits[1]),
