...
class LSGPRModelMBKMeans(LSGPRModel):
    ...
    def __init__(self, m_distribution=None, include_best=True, include_transfer=False, batch_size=1024,
        ...
        self.cluster = MiniBatchKMeans(n_clusters=self.m_points, batch_size=batch_size)
    ...
    def _train_sparse(self, atoms_list):
        ...
        try:
            full_update = self.get_episode_counter()%self.full_update_interval == 0
        except:
            full_update = True

        if hasattr(self.cluster, 'cluster_centers_') and self.sparsifier is None and not full_update:
            self._MB_episode()
        elif hasattr(self.cluster, 'cluster_centers_') and self.sparsifier is not None and self.sparse_update and not full_update:
            self._MB_episode()
        else:
        ...
        print('KMeans Episodes:', self.cluster.n_steps_)
    ...
    def _MB_episode(self):
        ...
        random_state = check_random_state(self.cluster.random_state)
