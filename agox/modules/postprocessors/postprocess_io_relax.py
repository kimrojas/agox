...
from agox.modules.helpers.bfgs_confine_atoms import BFGS_confine_atoms

optimizers = {'BFGS':BFGS, 'BFGS_confine_atoms':BFGS_confine_atoms, 'BFGSLineSearch':BFGSLineSearch}
...
from agox.modules.postprocessors.postprocess_io_relax import SecondaryPostProcessIORelax
...
from agox.modules.models.gaussian_process.delta_functions_multi.delta import delta
from agox.modules.models.model_wrap import get_wrapped_model_calculator
...
relaxer = SecondaryPostProcessIORelax(model, opt_run_kwargs, **settings)
...
class PrimaryPostProcessIORelax(PostprocessBaseClass):
    ...
    def process_list(self, candidates):
        ...
        t0 = dt()
        if self.get_episode_counter() < self.start_relax:
            return candidates
    ...
    def assign_from_main(self, main):
        ...
        # This ensures the secondary process is killed after the 'run' function of AGOX has finished. 
        # Otherwise this would just run until all wall-time has been used. 
        #         
        self.start_secondary_process()
        ...
        if self.model_training_mode == 'secondary':            
            print('Deleting observer as using secondary training mode')
            self.model.remove_as_observer_to_database(main.database)
            print('''WARNING: This does infact not work entirely yet, because the model is reattached later. 
                So for now you should manually remove the model training observer in the runscript: 
                agox.model_calculator.remove_as_observer_to_database(agox.database)
                or: 
                agox.database.delete_observer(agox.model_calculator.training_observer_func)
                If you are in secondary process training mode to avoid double training!
                This will not be required after further updates...
                ''')
...
class SecondaryPostProcessIORelax:
