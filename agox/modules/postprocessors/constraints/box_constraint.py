...
class BoxConstraint:
    ...
    def __init__(self, confinement_cell=None, cell_corner=None, indices=None, **kwargs):
        """
        ...
        self.cell_corner = np.array(cell_corner)
        if indices is None:
            indices = np.array([])
    ...
    def get_projection_coefficients_old(self, positions):
        return np.linalg.solve(self.confinement_cell, (positions-self.cell_corner).T).T

    def get_projection_coefficients(self, positions):
        positions = positions.reshape(-1, 3)
        return np.linalg.solve(self.effective_confinement_cell.T, (positions-self.cell_corner)[:, 0:self.dimensionality].T).T.reshape(-1, self.dimensionality)
    ...
    def get_confinement_limits(self):
        ...
        """
        conf = [self.cell_corner, self.confinement_cell @ np.array([1, 1, 1]) + self.cell_corner]
        return conf

    def todict(self):
        return {'name':'BoxConstraint', 
                'kwargs':{'confinement_cell':self.confinement_cell.tolist(), 'cell_corner':self.cell_corner.tolist(), 'indices':self.indices.tolist()}}
