...
class BehlerParrinello(DescriptorBaseClass):
    ...
    def __init__(self, species, eta, rs, rc=5, xi=[], lamb=[], eta_ang=[], **kwargs):
        """
        ...
        self.dimension = (self.num_radial+self.num_angular) * len(self.species)
    ...
    def create_local_features(self, atoms):
        ...
        for i in range(num_atoms):
            args = i1 == i
            for c, atomic_number in enumerate(self.species):
                indices = atoms.numbers[i2[args]] == atomic_number
                d = dists[args][indices].reshape(-1, 1)
