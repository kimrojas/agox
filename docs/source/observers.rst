...
The main loop, the one that runs over iterations, of the search function is in the AGOX class of main_agox.py written 
as such
...
.. literalinclude:: ../../agox/modules/helpers/helper_observers/iteration_timer.py
    :language: python
...
And at the end of each iteration it will print something like this:: 
...
to 'candidates'. Something has been added with that key by the collector, if the key did not match anything in the iteration 
...
.. note:: 
    ...
    The dictionary that underpins the data sharing is reset in each iteration! 
