...
for others it is. In the end it copies the files back to the created folders and moves the databases files out to the 
directory where the job was submitted. 

This particular script created 50 runs of the algorithm, and we may again use the batch analysis program to analyze the 
runs. 

.. code-block:: console

   python $AGOX_DIR/utils/batch_analysis.py -d small_gold_serial_1 small_gold_slurm/ -e -hg -dE 0.005

Which creates a figure like this one

.. figure:: slurm.png

Due to the higher number of independent runs of the algorithm the resolution of the reported statistics is higher, which 
